...
def make_stone(rivers, bg, length, N, rt, m):
    st = [bg]
    for i in range(1, length):
        st.append(create_vector_relative(st[i - 1], randint(-m, m), N / 15))
        if random() < rt and length - i > 1:
            rivers = make_stone(rivers, st[i - 1], length - i, N, rt * 0.9, int(m * 1.2))
    opi = st[0][:2]
...
def make_stones(map, M, length, rt, m):
    rivers = []
    N = map.shape[0]
    beg = begin_vc(N, M)
    for j in beg:
        rivers = make_stone(rivers, j, length, N, rt, m)
    return rivers
...
def add_stones(mapp, M, length, rt, m, rad):
    d = make_stones(mapp, M, length, rt, m)
    t = mapp
    for i in d:
        t = mark_cells_near_polyline(t, i, 1, radius=rad)
    return t
